...
const togglefinished=()=>{
 ...
 const handlecheckbox = (e) => {
  ...
  return (
    <>
      <Navbar />
      <div className="container m-auto md:w-[50%]  my-5 rounded-3xl p-5 bg-violet-100">
        ...
        <div>
          ...
          <input onChange={handlechange} value={task} className='w-[85%] h-11 my-3 rounded-full p-3' type="text" placeholder='Write your task'  onKeyDown={(e) => e.key === "Enter" && handleadd()} />
          <button onClick={handleadd} disabled={task.length<=3} className='bg-violet-700 text-white text-xl font-bold rounded-full w-[20%] h-11 m-3 items-center'>Save</button>
          <div>
        ...
        <div className="todos">
          ...
          {todos.map(item=>{
          return (showfinished|| !item.isCompleted)&& <div key={item.id} className="{task flex items-center w-full
           justify-between my-3}">
            <div className="flex gap-3">
            <input name={item.id} onChange={handlecheckbox} className='mx-2' type="checkbox" checked={item.isCompleted} id="" />
            <div className={item.isCompleted?"line-through":""}>{item.task}</div>
            </div>
            <div className="buttons flex h-full justify-end w-1/2 md:">
